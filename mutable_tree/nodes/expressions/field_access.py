...
class FieldAccess(Expression):
    def __init__(
        ...
        self.is_as_pattern = is_as_pattern
        self._check_types()

    def _check_types(self):
        if self.node_type != NodeType.FIELD_ACCESS:
            throw_invalid_type(self.node_type, self)
        # 如果是 as 模式，则跳过对象和字段的类型检查
        if not self.is_as_pattern:
            if (
                ...
                and self.object.node_type != NodeType.BINARY_EXPR
            ):
